...
Theorem nullable_means_empty : forall p s,
  PrefixTyped p s ->
  Nullable s ->
  EmptyPrefix p.
Proof.
...
Theorem nullable_means_maximal : forall p s,
  PrefixTyped p s ->
  Nullable s ->
  MaximalPrefix p.
Proof.
...
Theorem nullable_context_means_emptyon : forall eta g,
  NullableCtx g ->
  EnvTyped eta g ->
  EmptyOn (fv g) eta.
Proof.
...
- sfirstorder.
- sauto use:nullable_means_empty.
- sauto lq: on rew: off.
- sauto lq: on rew: off.
Qed.

Theorem nullable_context_means_maximalon : forall eta g,
  NullableCtx g ->
  EnvTyped eta g ->
  MaximalOn (fv g) eta.
Proof.
...
dependent induction H.
- sfirstorder.
- sauto use:nullable_means_maximal.
- sauto lq: on rew: off.
- sauto lq: on rew: off.
Qed.